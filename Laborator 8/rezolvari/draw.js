function handleClick(event)
{
   ...
   var color = document.getElementById('myColorInput').getAttribute('value');
   console.log(color);
...
function shiftRow(row, pos) 
{
   ...
   */
   ...
   let copyCells = Array.from(cells);
   
   for (var i = cells.length - 1; i - pos >= 0; i--)
   {
      var nextCell = (i - pos + cells.length) % cells.length;
      drawPixel(row, i, getComputedStyle(cells[nextCell]).backgroundColor);
   }

   for (var i = pos - 1, nextCell = copyCells.length - 1; i >= 0; i--, nextCell--)
   {
      drawPixel(row, i, getComputedStyle(copyCells[nextCell]).backgroundColor);
   }
}
...
window.onload = function()
{
   ...
   var colorInput = document.getElementById('myColorInput');
   colorInput.addEventListener('change', function(event) { 
                                                            var selectedColor = event.target.value; 
                                                            colorInput.setAttribute('value', selectedColor); 
                                                         } 
                              );

   var buttonCurcubeu = document.getElementById('buttonCurcubeu');
   buttonCurcubeu.addEventListener('click', function(event) { rainbow('verticala'); } );
   
   var buttonCurcubeu = document.getElementById('buttonClear');
   buttonCurcubeu.addEventListener('click', function(event) { drawRect(0, 0, 29, 29, 'white'); } );
}
